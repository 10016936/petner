...
<body>
	...
	<h3 align=center>주문 상품 확인</h3>
	<br>
	<table class=mytable align=center width=700 height=220>
		<tr>
	...
	<table class=mytable align=center width=700 height=50>
		<c:forEach var="row" items="${list}" varStatus="i">
			...
			<tr>
				...
				<td>${row.order_state}
						<c:if test="${order_product.order_state == '입금완료'}">
							<input type=button class="btn btn-outline-secondary" value="주문취소"
						...
						<c:if test="${order_product.order_state == '배송완료'}">
							<input type=button class="btn btn-outline-secondary" value="구매확정"
						...
						<c:if test="${order_product.order_state == '구매확정'}">
							<input type=button class="btn btn-outline-secondary" value="리뷰작성"
			...
			<tr colspan=4>
				...
				<th width=15%>재고</th>
				<td width=40%>&nbsp;${product_stock}&nbsp;
					<c:if test="${order_product.order_state == '입금완료'}">
							<input type=button class="btn btn-outline-secondary" value="배송시작"
	...
	<table class=mytable align=center width=700 height=200>
		<tr>
	...
	<table class=mytable align=center width=700 height=300>
		<tr>
	...
	<div align="center">
	<c:if test="${sessionScope.mem_type == '사업자'}">
		<input type="button" class="btn btn-outline-secondary" value="목록" onClick="location.href='sell_list'">
	</c:if>
	<c:if test="${sessionScope.mem_type == '일반회원' || sessionScope.mem_type == '일반'}">
		<input type="button" class="btn btn-outline-secondary" value="목록" onClick="location.href='order_list'">
	</c:if>
	</div>
	<br>
	</div>

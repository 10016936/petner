...
<body>
	<div class="container" align="center">
		...
		<form action="lostUpdateFormOk" method="post" name="frm"
			...
			<table class="mytable" align=center width=600 height=600>
				<tr>
					<th width=150>작성자</th>
					<td>${lost.mem_id}</td>
				</tr>
				<tr>
					<th height=50%>잃어버린 위치</th>
					<td>
									 <c:if test="${lat==null}">
...
kakao.maps.event.addListener(map, 'click', function(mouseEvent) {        
    ...
    var message = '클릭한 위치의 위도 : ' + latlng.getLat() + '<br>경도 : ' + latlng.getLng();
...
function myson(mem_id,lost_title,lost_content,lost,lost_pwd,lost_file){
//	alert("lat2:"+lat);
//	alert("lng2:"+lng);
	/* var lat = latlng.getLat();
	var lng = latlng.getLng(); */
...
//	 var mem_id = document.getElementById("mem_id") 
//	var mem_id = $('#mem_id').val();
	 location.href="lostlatlng2?lat="+lat+"&lng="+lng+"&mem_id="+mem_id+"&lost_title="+lost_title+"&lost_content="+lost_content+"&lost="+lost+"&lost_pwd="+lost_pwd+"&lost_file="+lost_file; 
	/* location.href="insertAction?lat="+lat+"&lng="+lng; */
	/* document.getElementById(lat).value = lat; 
	document.getElementById(lng).value = lng; */ 
//	$("#lat").val(lat);
//	$("#lng").val(lng);
	/* self.close(); */
					...
					<input type="button" value="위치수정 확인" onclick="myson('${mem_id}','${lost.lost_title}','${lost.lost_content}','${lost}','${lost.lost_pwd}','${lost.lost_file}')"/>
					</td>				
				</tr>
				<tr>
					<th>비밀번호</th>
					<td><input type="password" size=60 name="lost_pwd2" id="lost_pwd2"required="required"
								></td>
				</tr>
				<tr>
					<th>제목</th>
					<td><input type="text" size=60 name="lost_title" id="lost_title" required="required"
								value="${lost.lost_title}"></td>
				</tr>
				<tr>
					<th height=23%>내용</th>
					<td><textarea rows="8" cols="57" name="lost_content" id="lost_content"
			...
			<div align=center>
				...
				<input type="button" class="btn btn-outline-secondary" value="취소" onClick="history.back(-1)">
			</div>
		</form>
	</div>
	<br>

...
<body>
	...
	<input type="hidden" name="mem_id" value="${lost.mem_id}"> <!-- 로그인해서 댓글을 다는 아이디의 아이디값 -->
		...
		<h2 class="bbslist_title">댓글 목록</h2>
		 <table align=center id="bbslist_t" width="1000" > 
			<tr valign="middle" bordercolor="#333333">
				<th style="font-family: Tahoma; font-size: 11pt;" width="20%" height="26">
			...
			<tr valign="middle" bordercolor="#333333"
				...
				<td style="font-family: Tahoma; font-size: 10pt;">
					<div align="center">
					<fmt:formatDate value="${b.lostreply_date}" pattern="yy-MM-dd HH:mm"/>										
					</div>
				</td>
				<td id="btn_${b.lostreply_no}" >
							<input type="button" class="btn btn-outline-secondary" value="댓글달기" onclick="rreply(${b.lostreply_no})">
					...
					<%-- <input type="button" value="수정" class="edit1" id="${b.lostreply_no}"> --%> <!-- rb.rno  댓글번호, 각자의 댓글을 구별하기 위해서 사용-->							
					<input type="button" class="btn btn-outline-secondary" value="삭제"	 onclick="del(${b.lostreply_no},${b.lost_no})"> <!-- rno자식의 글번호(댓글) bno부모의 글번호 -->
					</c:if>
					</td>			
			</tr>
			...
			<td></td>
			<td><div style="display:none" id="son2_${b.lostreply_no}">
				<textarea id="son3_${b.lostreply_no}" rows="4" cols="30" name="lostreply_content"></textarea></div></td>
			<td><div style="display:none" id="son4_${b.lostreply_no}">
				<input type="button" class="btn btn-outline-secondary" value="댓글입력" id="son4_${b.lostreply_no}"

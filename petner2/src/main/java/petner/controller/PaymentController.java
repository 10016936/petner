...
public class PaymentController {
	...
	public String paymentstart(@RequestParam("mem_id") String mem_id, @RequestParam("product_no") int product_no,
			@RequestParam("product_ea") int product_ea, Model model, HttpSession session) {
...
// 		폼으로 넘기기 전에 여기서 상품가격 x 수량을 변수 payment_price변수를 만들어서 저장한후 모델을 통해서 뿌려주어야 한다.		
	...
	// 체크박스가 눌리면서 insert -> javascript로 실행하는데 이상하게 payment dto로 못받아 오길래 일단 직접 값을넣어줌
	@RequestMapping(value = "paymentapi", method = RequestMethod.POST)
	public String paymentapi(Payment payment, String mem_id, String mem_post, String mem_addr1, String mem_addr2,
			int payment_price, int product_no, int product_ea, int product_price, String product_name,
			String payment_info, int seller_no, Model model) {
...
//		int payment_no1 = payment.getPayment_no();
	...
	public String membermiletest(int mem_point, int usemem_point, int product_no, int product_ea, Model model,
...
//		int product_ea = 2;
	...
	public String basketinsertstart1(Payment payment, String mem_id, String mem_post, String mem_addr1,
			String mem_addr2, int payment_price, int product_no, String payment_info, int totalprice, String pro_name,
			int seller_no, Model model) {
